...
class PanopLiDataset(BaseDataset):
    ...
    def load_sample(self, sample_index):
        ...
        if image.shape[-1] == 4:  # Assuming the shape is (H, W, C) and C=4 indicates RGBA
            image = image[:, :, :3]  # Remove the alpha channel

        # Reshape the image to the expected format
        ...
        if self.load_depth:
            raw_depth = np.array(Image.open(self.root_dir / "depth" / f"{self.all_frame_names[sample_index]}.png").convert('L'))
