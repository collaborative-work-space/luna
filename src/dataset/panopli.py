...
class PanopLiDataset(BaseDataset):
    ...
    def __init__(self, root_dir, split, image_dim, max_depth, overfit=False, num_val_samples=8, load_depth=True, load_feat=False, instance_dir='filtered_instance', semantics_dir="filtered_semantics",
    ...
    def load_sample(self, sample_index):
        ...
        semantics = torch.from_numpy(np.array(semantics.resize(self.image_dim[::-1], Image.NEAREST), dtype=np.uint8)).long()
        ...
        instances = torch.from_numpy(np.array(instances.resize(self.image_dim[::-1], Image.NEAREST), dtype=np.uint8)).long()
        # noinspection PyTypeChecker
        ...
        if self.load_depth:
            raw_depth = np.array(Image.open(self.root_dir / "depth" / f"{self.all_frame_names[sample_index]}.png")).astype(np.float32)
            raw_depth /= 1000.0
            raw_depth[raw_depth > (self.max_depth / self.normscene_scale.item())] = (self.max_depth / self.normscene_scale.item())
            depth_cam = torch.from_numpy(np.array(Image.fromarray(raw_depth).resize(self.image_dim[::-1], Image.BILINEAR)))
        ...
        room_mask_path = self.root_dir / "invalid" / f"{self.all_frame_names[sample_index]}.png"
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'val' or self.split == 'test':
            ...
            semantics = torch.from_numpy(np.array(semantics.resize(self.image_dim[::-1], Image.NEAREST), dtype=np.uint8)).long().reshape(-1)
            instances = torch.from_numpy(np.array(instances.resize(self.image_dim[::-1], Image.NEAREST), dtype=np.uint8)).long().reshape(-1)
...
class InconsistentPanopLiSingleDataset(PanopLiDataset):
    ...
    def __init__(self, root_dir, split, image_dim, max_depth, overfit=False, num_val_samples=8, max_rays=512, semantics_dir='filtered_semantics',
                 instance_dir='filtered_instance_inc', instance_to_semantic_key='instance_to_semantic_inc', create_seg_data_func=create_segmentation_data_base, subsample_frames=1, load_depth=True):
        super().__init__(root_dir, split, image_dim, max_depth, overfit, num_val_samples, semantics_dir=semantics_dir,
                         instance_dir=instance_dir, instance_to_semantic_key=instance_to_semantic_key, create_seg_data_func=create_seg_data_func, subsample_frames=subsample_frames, load_depth=load_depth)
        print('Preparing InconsistentPanopLiDataset...')
...
class SegmentPanopLiDataset(PanopLiDataset):
    ...
    def __init__(self, root_dir, split, image_dim, max_depth, overfit=False, num_val_samples=8, max_rays=512, semantics_dir='filtered_semantics',
        ...
        for i in range(len(self.train_indices)):
            ...
            segments = torch.from_numpy(np.array(segments.resize(self.image_dim[::-1], Image.NEAREST), dtype=np.uint8)).long().reshape(-1)
