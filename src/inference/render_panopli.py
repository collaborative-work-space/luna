...
def render_panopli_checkpoint(config, trajectory_name, test_only=False):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(trajectory_loader)):
            ...
            p_depth = (p_depth - p_depth.min()) / (p_depth.max() - p_depth.min() + 1e-5)
            ...
            stack = visualize_panoptic_outputs(p_rgb, p_semantics, p_instances, p_depth, None, None, None, None, H, W, thing_classes=test_set.segmentation_data.fg_classes, visualize_entropy=True)
            ...
            frame_index = test_set.val_indices[batch_idx]
            xname = f"0_{frame_index:04d}.png"
            p_rgb_np = p_rgb.view(H, W, 3).cpu().numpy()  # in [0,1] float
            pred_rgb_dir = output_dir / "pred_rgb_raw"
            pred_rgb_dir.mkdir(exist_ok=True)
            np.save(pred_rgb_dir / xname.replace(".png", ".npy"), p_rgb_np)
